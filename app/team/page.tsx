...
export default function TeamPage() {
  ...
  return (
    <main className="relative overflow-hidden bg-white text-slate-900 min-h-screen dark:bg-[#060818] dark:text-white">
      ...
      <section className="relative pt-32 pb-20 px-4 md:px-8">
        <div className="container mx-auto max-w-7xl text-center">
          <h1 className="text-4xl md:text-6xl font-bold text-slate-900 dark:text-white mb-6">
            Meet Our <span className="text-violet-600 dark:text-violet-500">Team</span>
          </h1>
          ...
          <div className="flex justify-center gap-4 mt-8">
            <div className="flex -space-x-4">
              {teamMembers.slice(0, 5).map((member) => (
                <div
                  key={member.id}
                  className="w-12 h-12 rounded-full border-2 border-violet-500/50 overflow-hidden relative shadow-sm"
                >
                  <Image
                    src={member.image}
                    alt={member.name}
          ...
          {Object.entries(departments).map(([department, members]) => (
            <div key={department} className="mb-16">
              <h2 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-8">
                {department}
                <span className="text-violet-600 dark:text-violet-400 text-sm font-semibold ml-3">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {members.map((member) => (
                  <div
                    key={member.id}
                    className="group rounded-2xl p-8 hover:transform hover:scale-[1.02] transition-all duration-300 border border-slate-200 bg-white/90 shadow-sm dark:border-violet-500/10 dark:bg-[#0F1438] dark:shadow-none hover:shadow-lg hover:shadow-violet-500/10"
                  >
                    {/* Card content with horizontal layout */}
                    <div className="flex flex-col sm:flex-row items-start gap-6">
                      {/* Image - larger */}
                      <div className="relative w-32 h-32 rounded-full overflow-hidden border-2 border-violet-500/30 group-hover:border-violet-500 transition-colors flex-shrink-0 mx-auto sm:mx-0 shadow-sm">
                        <Image
                          src={member.image}
                          alt={member.name}
                          fill
                      ...
                      <div className="flex-1 min-w-0 w-full">
                        <h3 className="text-2xl font-semibold text-slate-900 dark:text-white group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors text-center sm:text-left">
                          {member.name}
                        </h3>
                        <p className="text-violet-600 dark:text-violet-400 text-base font-semibold mb-2 text-center sm:text-left">
                          {member.role}
                        </p>
                        <p className="text-slate-600 dark:text-slate-300 text-sm mb-3 line-clamp-3 text-center sm:text-left">
                          {member.bio}
                        </p>

                        {/* Experience */}
                        <p className="text-slate-500 dark:text-slate-400 text-sm mb-3 text-center sm:text-left font-medium">
                          ⚡ {member.experience}
                        </p>
                        ...
                        <div className="flex flex-wrap gap-2 mb-4 justify-center sm:justify-start">
                          {member.skills.map((skill) => (
                            <span
                              key={skill}
                              className="text-xs px-3 py-1 rounded-full bg-violet-50 text-violet-700 font-medium border border-violet-200 dark:bg-violet-500/10 dark:text-violet-300 dark:border-violet-500/20"
                            >
                              {skill}
                            </span>
                          ))}
                        ...
                        {/* Social Links */}
                        <div className="flex gap-4 pt-3 border-t border-slate-200 dark:border-violet-500/10 justify-center sm:justify-start">
      ...
      <section className="py-20 px-4 md:px-8">
        <div className="container mx-auto max-w-4xl">
          <div className="bg-white rounded-2xl p-8 md:p-12 text-center border border-slate-200 transition-colors duration-300 dark:bg-[#0F1438] dark:border-violet-500/20">
